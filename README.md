...
- 首页：知识图谱浏览器，呈现 51 个概念、125 条关系，支持拖拽、缩放、筛选、节点聚焦和 visual metaphor。
...
- ✅ 图谱 Tier 1/2：补齐 40 条横向 related_to 关系，语义安全地补 3 条 prerequisite_of，不破坏主路径 DAG
- ✅ 图谱边治理：每条边带 `group`、`critical`、`source` 元数据；学习路径只显示主干依赖和 4 条关键补链；外部 reference 边可通过脚本合并

仍待补齐：
1. 核心链路（约7个课时）：循环小数、长方体体积、容积、约分通分、分数加减法、组合图形面积、折线统计图
2. 高频交互组件迭代：分数比较条（动画切换）、更细的统计图交互、应用题策略树
3. 诊断题 Tier 3：等待完整 153 题 reference 后接入
4. 继续扩展 visual metaphor 图像资产
